...
| `-dev, --device <dev1,dev2,..>` | comma-separated list of devices to use for offloading (none = don't offload); with --fit, devices are considered in list order (higher priority first)<br/>use --list-devices to see a list of available devices<br/>(env: LLAMA_ARG_DEVICE) |
| `--layer-devices, --layer-device <dev0,dev1,..>` | comma-separated list of devices for each model layer in layer order (none = CPU; output layer follows the last entry if omitted)<br/>use --list-devices to see a list of available devices<br/>(env: LLAMA_ARG_LAYER_DEVICES) |
...
| `-fitt, --fit-target MiB0,MiB1,MiB2,...` | target margin per device for --fit, comma-separated list of values in --device order, single value is broadcast across all devices, default: 1024<br/>(env: LLAMA_ARG_FIT_TARGET) |
